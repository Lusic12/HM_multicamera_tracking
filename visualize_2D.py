...
class DataReader:
    ...
    def __init__(self, image_dir, label_dir, num_cam, start_frame=0, end_frame=0, save_videos=False):
        ...
        if self._save_videos:
            ...
            for cam_id in range(1, self._num_cam + 1):
                self._rgb_videos[cam_id] = cv2.VideoWriter('video_camera_view_' + str(cam_id) + '.avi',
                                                           cv2.VideoWriter_fourcc(*'MJPG'), 15, (640, 360))
    ...
    def read(self, frame_id):
        ...
        for cam_id in range(1, self._num_cam + 1):
            ...
            label = json.load(
                open(os.path.join(self._label_dir, 'rgb_' + str(frame_id).zfill(5) + '_' + str(cam_id) + '.json')))
            rgbs[cam_id] = rgb
            labels[cam_id] = label
        ...
        for frame_id in range(self._start_frame, self._end_frame):
            ...
            for cam_id in range(1, self._num_cam + 1):
                ...
                for id, (x_min, y_min, x_max, y_max) in tracklet.items():
                    ...
                    rgb = cv2.putText(rgb, str(id), (int(x_min), int(y_min)), cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), 2,
                                      cv2.LINE_AA)
                rgb = cv2.putText(rgb, str(frame_id), (30, 30), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 0), 2,
                                  cv2.LINE_AA)
                if self._save_videos:
...
if __name__ == '__main__':
    ...
    label_dir = 'train/labels/63am/cafe_shop_0'
    num_cam = 4
    start_frame, end_frame = 0, 3000
    data_reader = DataReader(image_dir, label_dir, num_cam, start_frame, end_frame, save_videos=True)
    data_reader.plot()
